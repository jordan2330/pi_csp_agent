...
## 数据源调研结论（2026-09-23，v4 已落地）

### 背景

原痛点：药物分类准确性不够（临床试验登记数据里没有注册分类），此前用博查搜索"猜"，命中率约 31%。
调研目标：能否用已付费的医药魔方 PharmaGO 替代 CDT + 博查。

### 结论一：PharmaGO 自动化路线被合同否决（已放弃）

- PharmaGO/TrialiCube 能力实测足够（`go.pharmcube.com/drugct` 中国临床、`ct.pharmcube.com/trial` 全球临床试验；支持试验药品/靶点/适应症/申办方/登记平台/多种日期筛选 + 自定义列 + 导出；微信扫码登录）
- **但《用户服务协议》第 2.4 条明令禁止**："所有通过机械化、程序化、自动化、脚本式或任何其他自动化方式访问和使用数据库及信息的行为均被严格禁止……否则……临时关闭部分或全部数据库使用权限……或直接终止提供服务，且所有已支付款项将不予退还"
- 第 2.2 条允许"导出限定数量数据信息"用于"内部管理、援引及参考"，但**禁止**"通过对本网站或本服务直接或间接创建或编纂数据库"
- → **禁止对 pharmcube 系站点写爬虫**；其人工导出结果只能做交叉校验，不长期囤积
- 另：医药魔方开放平台/MCP（`open.pharmcube.com`，含中国临床试验 API 与 MCP 服务，`mcp-openapi.pharmcube.com/mcp`）报价 20 万/年 → 已排除

### 结论二：CDE 官方受理品种信息 = 分类富化的最优数据源（已接入）

- 入口：`https://www.cde.org.cn/main/xxgk/listpage/9f9c74c73e0f8f56a8bfbc646055026d`（政府信息公开，与 CDT 同性质，无自动化禁止条款）
- 字段：`受理号 | 药品名称 | 药品类型 | 申请类型 | 注册分类 | 企业名称 | 承办日期`（**直接给出注册分类**）
- 实测：1.5-3 秒/品种，无反爬质询；177 个品种约 10 分钟；有信号命中率 ~80%（vs 博查 31%）
- 落地：`scripts/lib/cde-classify.js` + `config/cde-classify.json` + `config/cde_class_cache.json`；Phase 2c 主源，博查降为兜底
- 经验教训（踩过的坑，写死在 SKILL.md）：必须定位**可见**表单（页面有 22 个"查询"按钮）、识别**默认列表指纹**防止"查询未生效"被当成结果、空结果必须等 2.5 秒确认

### 结论三：CT.gov REST API 保留

免费、结构化、字段比导出全（NCT/期次/干预/日期），没有理由为"数据源统一"而放弃。

### 后续可选（未决策）

- 已上市企业/批文（`datasearch.nmpa.gov.cn` 两条门槛见上一节；CDE「上市药品信息」+「药品目录集信息」子库可覆盖大部分需求，且免费——**若要做，优先评估 CDE 这条路而非商业 API**）
- 摩熵数科开放平台 API 询价（`open.bcpmdata.com`，中国临床试验接口含药品名称/药物类型/靶点/适应症字段）——仅当 CDE 路走不通时再考虑
