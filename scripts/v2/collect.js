 ...
 *
 * 用法: node scripts/v2/collect.js [--db config/csp.db] [--scenario nitrosamine]
 *
...
async function main() {
  ...
  // 兼容旧键名恢复游标（v1 曾存于缓存 JSON 的 last_cdt_regno，v2 起游标只信任 meta）
  const fda = (() => {
  ...
  const apis = db.prepare('SELECT en_name, cn_name FROM fda_apis ORDER BY en_name').all();
  ...
  // ── CDT 阶段 ──
  log('═══ CDT ═══');
  const minYear = cutoff.slice(0, 4).padEnd(4, '0');
  let cdtResult = null;
  try {
    ...
    cdtResult = await runCDT(db, cdtApis, { workerCount: 2, minYear: parseInt(minYear), log });
    db.prepare('INSERT INTO meta (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value')
      .run('cdt_as_of', today());
  } catch (e) {
